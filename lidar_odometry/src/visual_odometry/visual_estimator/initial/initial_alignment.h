...
public:
    ...
    odometryRegister(std::shared_ptr<rclcpp::Node> node_in)
    {
        ...
        node = node_in;
        pub_latest_odometry = node_in->create_publisher<nav_msgs::msg::Odometry>("odometry/test", 1000);
    }

    // convert odometry from ROS Lidar frame to VINS camera frame
    vector<float> getOdometry(deque<nav_msgs::msg::Odometry>& odomQueue, double img_time, int cam_id)
    {
        tf2::Quaternion q_cam_to_lidar;
        ...
        {
            if (odomQueue.front().header.stamp.sec + odomQueue.front().header.stamp.nanosec *  1e-9< img_time - 0.05)
                odomQueue.pop_front();
            else
                break;
        }
        ...
        for (int i = 0; i < (int)odomQueue.size(); ++i)
        {
            ...
            if (odomCur.header.stamp.sec + odomCur.header.stamp.nanosec * 1e-9< img_time - DELTA_T_IMU) // imu frequency
                continue;
            else
                break;
        }
        ...
        if (abs(odomCur.header.stamp.sec + odomCur.header.stamp.nanosec * 1e-9 - img_time) > 0.05)
        {
            return odometry_channel;
        }
        ...
        tf2::Quaternion q_pi;
        q_pi.setRPY(0, 0, M_PI);
        q_odom_cam = q_pi * (q_odom_lidar * q_cam_to_lidar);
        ...
        // TODO: SE NON FUNZIONA TOGLIERE I COMMENTI A QUESTE RIGHE E PUBBLICARE L'ODOMETRIA DI VINS PER VEDERE DOVE STA IL PROBLEMA
        // TODO da qui era tutto commentato
        //odomCur.header.stamp = ros::Time().fromSec(img_time);
        //odomCur.header.stamp = rclcpp::Time(static_cast<int64_t>(img_time * 1e9) );
