...
public:
    ...
    void imuOdometryHandler(const nav_msgs::msg::Odometry::SharedPtr odomMsg)
    {
        ...
        // RCLCPP_INFO(this->get_logger(), "IMU odometry position (x,y,z)= (%.2f,%.2f,%.2f)", t.transform.translation.x,t.transform.translation.y,t.transform.translation.z);
...
public:
    ...
    void imuHandler(const sensor_msgs::msg::Imu::SharedPtr imu_raw)
    {
        ...
        scaleImuAcceleration(imu_raw);

        if (!imu_tracker_init)
        {
            imu_tracker_init = true;

            imu_tracker_.reset(new ImuTracker(10.0, stamp2Sec(imu_raw->header.stamp)));
        }
        sensor_msgs::msg::Imu thisImu = imuConverter(*imu_raw, imu_tracker_);
